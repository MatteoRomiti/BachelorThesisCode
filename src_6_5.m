%calcolo delle perdite con la (6.15)
...
L0 = loss_function_Nphi(EN(1),Nv, Nphi, Rb, ratio);

L3 = loss_function_Nphi(EN(4),Nv, Nphi, Rb, ratio);

L6 = loss_function_Nphi(EN(7),Nv, Nphi, Rb, ratio);

plot(indScint, L0, indScint, L3, indScint, L6, 'LineWidth',1.5)
xlabel('indice di scintillazione')
ylabel('Loss[dB]')
title (['E[v*E[cos(\phi)]], R_b=', num2str(Rb), ', R_b/B_l=', num2str(ratio), ', N_v = ', num2str(Nv), ', N_\phi = ', num2str(Nphi)])
